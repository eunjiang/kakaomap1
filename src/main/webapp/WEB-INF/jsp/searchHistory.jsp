...
<title>검색 히스토리 - Kakao map search</title>
	...
	<div class="container mt-3">
		<div class="row">
			<div class="col list-bookmark">
				<h3>검색 히스토리</h3>
				<form action="./searchHistory" id="searchForm" name="searchForm"
					method="get">
					<input type="hidden" name="page"
						value="${searchHistoryPage.number}"> <input type="hidden"
						name="size" value="${searchHistoryPage.size }">
					<div class="form-row">
						<div class="form-group">
							<select id="select-sort" name="sort" class="form-control">
								<option value="regdate,desc"
									${param.sort=='regdate,desc'?'selected="selected"':'' }>시간역순
								</option>
								<option value="regdate,asc"
									${param.sort=='regdate,asc'?'selected="selected"':'' }>시간순
								</option>
							</select>
						</div>
					</div>
				...
				<ul class="list-group">
					...
					<c:forEach var="b" items="${searchHistoryPage.content }">
						<li class="list-group-item"><dl>
								...
								<dd>
									<span class="blockquote-footer text-right"><fmt:formatDate
											value="${b.regdate }" pattern="yyyy. MM. dd HH:mm:ss" /></span>
								</dd>
							</dl></li>
					</c:forEach>
				</ul>
